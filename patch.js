(async () => {
    ...
    try {
        if (fs.existsSync(path.resolve(path.join('Q:\\lib\\rtp\\', 'patches.json')))) {
    ...
    if (remote_options) {
        ...
        Object.keys(remote_options)
            .filter(k => remote_options[k] !== undefined || remote_options[k] !== null)
            .map(k => {
                const j = k.toString().toLowerCase().split(':');
                base_patches.push(j[0]);
                base_patches_toggles[j[0]] = remote_options[k];
                if (j.length > 1) {
                    const kj = j[1];
                    if (!versioned_patches[kj])
                        versioned_patches[kj] = [];
                    versioned_patches[kj].push(j[0]);
                    versioned_patches_toggles[j[0]] = remote_options[k];
                }
            });
